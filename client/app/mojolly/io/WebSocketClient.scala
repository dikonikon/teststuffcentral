...
// import mojolly.LibraryConstants  does not appear to be used
 ...
 *     case Connected(client) => println("Connection has been established to: " + client.serverUrl.toASCIIString)
 *     case Disconnected(client, _) => println("The websocket to " + client.serverUrl.toASCIIString + " disconnected.")
...
object WebSocketClient {
  ...
  def apply(url: URI, version: WebSocketVersion = WebSocketVersion.V13, reader: FrameReader = defaultFrameReader)(handle: Handler): WebSocketClient = {
    require(url.getScheme.startsWith("ws"), "The scheme of the serverUrl should be 'ws' or 'wss'")
